...
void test_basic_report() {
  ...
  const struct zw_command_class* cls =
      zw_cmd_tool_get_cmd_class(NULL, zw_cmd_tool_class_name_to_num("COMMAND_CLASS_BASIC"), 0);
...
void test_windows_corvering_set() {
  uint8_t frame[100];
  const struct zw_command_class* cls =
      zw_cmd_tool_get_cmd_class(NULL, zw_cmd_tool_class_name_to_num("COMMAND_CLASS_WINDOW_COVERING"), 0);
  const struct zw_command* cmd =
