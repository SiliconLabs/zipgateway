 ...
 * The Z/IP connection module implements the logic in Z/IP UDP frame transmission
 *and reception.
 *
 * @{
 */

typedef struct zconnection zconnection_t;
...
typedef enum {
  ...
  TRANSMIT_WAIT,    //!< Transmission was not completed due to NAck+Waiting.
 ...
 * @return True if this is a response to another frame,  i.e., a report sent as a
...
/**
 * Get the busy state of the connection.
 *
 * @param connection A handle to the connection object
 ...
 *
 * @param connection A handle to the connection object
 * @param remote_addr Pointer to the buffer that will receive the remote endpoint information
 */
 ...
 *
 * @param connection A handle to the connection object
 * @return Pointer to a user owned buffer associated with the zconnection
 */
