...
static void browse_callback(AvahiServiceBrowser *b, AvahiIfIndex interface,
  ...
  switch (event) {
    ...
    case AVAHI_BROWSER_NEW:
      ...
      /* Create a service resolver to subscribe to data changes for this
       * service. For each change resolve_callback will be called with
       * AVAHI_RESOLVE_FOUND, and userdata = n. */
...
void *zresource_mdns_thread_func(void *user) {
  ...
  if (!(simple_poll = avahi_simple_poll_new())) {
    fprintf(stderr, "Failed to create Avahi mDNS simple poll object.\n");
    goto fail;
  }
  ...
  if (!client) {
    fprintf(stderr, "Failed to create Avahi mDNS client: %s\n", avahi_strerror(error));
    goto fail;
  }
  /* Create the service browser. browse_callback will be called whenever a new
   * service appears or is removed from the network.
   */
  if (!(sb = avahi_service_browser_new(client, AVAHI_IF_UNSPEC,
                                       AVAHI_PROTO_UNSPEC, "_z-wave._udp", NULL,
                                       0, browse_callback, client))) {
    fprintf(stderr, "Failed to create Avahi mDNS service browser: %s\n",
