...
bool ZW_NVM_Backup(const char* filename,uint8_t chiptype)
{
  ...
  */
  ...
  while (2 != read_status)
  {
    ...
    /* Print the progress*/
    fflush(stdout);
    printf("\rReading %d bytes...",offset);
  }
  printf("\n");
 ...
 /* On 700/800-series chips, SerialAPI_nvm_close must be followed by a chip reset */
...
bool ZW_NVM_Restore(const char* filename, uint8_t chiptype)
{
  ...
  */
  ...
  while (0 != length_write)
  {
    ...
    {
      printf("Warning: Something went wrong when writing NVM. Aborting, length_written: %d, length_write: %d\n", length_written, length_write);
      break;
    }
  ...
  *in 700/800 series: the FUNC_ID_ZW_WATCHDOG_ENABLE will be removed, therefore, ZW_SoftReset() should be used*/
  if (ZW_GECKO_CHIP_TYPE(chiptype)) {
    ZW_SoftReset();
  }
  else {
